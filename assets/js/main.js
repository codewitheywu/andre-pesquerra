...
(function () {
  ...
  /* ── Background Music (opt-in, local file) ────────────────────
     Never autoplays and is deliberately quiet. The audio file is only
     requested after the visitor presses the dock, and the dock itself only
     appears when the file actually exists on the server (so the feature
     stays invisible until assets/audio/coffee.mp3 is added). ──────── */
  const musicDock = document.getElementById('music-dock');
  if (musicDock) {
    const SRC       = musicDock.dataset.src;
    const VOLUME    = 0.2;    // linear gain; ~-14 dB, a soft bed under the page
    const FADE_IN   = 2000;   // ms
    const FADE_OUT  = 500;    // ms
    const status    = document.getElementById('music-status');
    const saveData  = !!(navigator.connection && navigator.connection.saveData);

    let audio = null;         // created on first press
    let ctx = null;           // Web Audio, only where element volume is read-only (iOS)
    let gainNode = null;
    let fadeTimer = null;
    ...
    const setState = (next) => {
      state = next;
      const active = next === 'loading' || next === 'playing';
      musicDock.dataset.state = next;
      musicDock.setAttribute('aria-pressed', active ? 'true' : 'false');
      musicDock.setAttribute('aria-busy', next === 'loading' ? 'true' : 'false');
      musicDock.setAttribute('aria-label',
        (active ? 'Pause' : 'Play') + ' background music: Coffee by Jack Stauber');
    };

    // iOS Safari ignores HTMLMediaElement.volume (always 1), so there we route the
    // element through a GainNode to keep the level low.
    const volumeIsSettable = () => {
      const probe = new Audio();
      probe.volume = 0.5;
      return probe.volume === 0.5;
    };
    const getLevel = () => (gainNode ? gainNode.gain.value : audio.volume);
    const setLevel = (v) => {
      const level = Math.min(1, Math.max(0, v));
      if (gainNode) gainNode.gain.value = level; else audio.volume = level;
    };
    // setInterval, not requestAnimationFrame: rAF is paused in background tabs.
    ...
    const teardown = (msg) => {
      ...
      if (ctx) { ctx.close(); ctx = null; gainNode = null; }
      setState('idle');
      if (msg) say(msg);
    };
    ...
    const build = () => {
      ...
      audio.loop = true;
      audio.preload = 'auto';
      audio.addEventListener('error', () => teardown('Music is unavailable right now.'));
      if (volumeIsSettable()) {
        audio.volume = 0;
      } else {
    ...
    const play = (fadeMs) => {
      ...
      if (started && started.then) {
        ...
        }).catch(() => teardown('Music could not start. Press play to try again.'));
      }
    };

    musicDock.addEventListener('click', () => {
      if (state === 'idle') {
        build();
        setState('loading');
        play(FADE_IN);
        ...
        setState('paused');
        say('Music paused');
        fadeTo(0, FADE_OUT, () => { if (audio && state === 'paused') audio.pause(); });
      } else if (state === 'paused') {
        setState('loading');
        play(FADE_IN / 2);
      } else {                      // loading: a second press cancels
        teardown('Music cancelled');
      }
    });

    // Reveal the dock only if the file exists (and the visitor isn't on Save-Data).
    if (SRC && !saveData && window.fetch) {
      fetch(SRC, { method: 'HEAD' }).then((res) => {
        const type = res.headers.get('content-type') || '';
        if (res.ok && /audio|mpeg|ogg|wav/i.test(type)) musicDock.hidden = false;
      }).catch(() => {});
    }
  }
